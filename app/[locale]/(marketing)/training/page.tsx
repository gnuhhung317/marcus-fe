import { ArrowRight, BookOpenText, Rocket, Waypoints } from 'lucide-react';
...
export default async function TrainingPage() {
  ...
  const roadmap = [
    {
      icon: BookOpenText,
      title: t('roadmap.curriculum.title'),
      description: t('roadmap.curriculum.description'),
    },
    {
      icon: Waypoints,
      title: t('roadmap.labs.title'),
      description: t('roadmap.labs.description'),
    },
    {
      icon: Rocket,
      title: t('roadmap.enrollment.title'),
      description: t('roadmap.enrollment.description'),
    },
  ];
          ...
          <div className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              <Button asChild variant="outline" className="rounded-xl px-6 py-3 text-sm font-bold uppercase tracking-[0.14em]">
                <Link href="/login?next=/terminal">{t('secondaryCta')}</Link>
              </Button>
            </div>
          </div>
          ...
          <Card variant="glass-strong" className="p-6">
            ...
            <div className="mt-6 space-y-3">
              <div className="rounded-2xl border border-border/70 bg-surface/70 p-4">
                <p className="text-[11px] uppercase tracking-[0.16em] text-muted">{t('status.curriculumLabel')}</p>
                <p className="mt-2 text-sm font-semibold text-main">{t('status.curriculumValue')}</p>
              </div>
              <div className="rounded-2xl border border-border/70 bg-surface/70 p-4">
                <p className="text-[11px] uppercase tracking-[0.16em] text-muted">{t('status.labsLabel')}</p>
                <p className="mt-2 text-sm font-semibold text-main">{t('status.labsValue')}</p>
              </div>
              <div className="rounded-2xl border border-border/70 bg-surface/70 p-4">
                <p className="text-[11px] uppercase tracking-[0.16em] text-muted">{t('status.enrollmentLabel')}</p>
                <p className="mt-2 text-sm font-semibold text-main">{t('status.enrollmentValue')}</p>
              </div>
            </div>
          </Card>
        </div>
