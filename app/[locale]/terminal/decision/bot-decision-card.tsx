...
export function BotDecisionCardComponent({ card, onKeep, onUnsubscribe, isBusy = false, isKept = false }: BotCardProps) {
  ...
  return (
    <article className="glass-strong h-full rounded-2xl border border-border shadow-[var(--shadow-soft)]">
      <div className="flex h-full flex-col p-5">
        <div className="flex-1 space-y-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className={`rounded-full border border-border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] ${tone.badge}`}>
              {tone.label}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full border border-border bg-surface px-2.5 py-1 text-[11px] font-semibold text-fg-muted">
              {card.status}
            </span>
            <span className="rounded-full border border-border bg-surface px-2.5 py-1 text-[11px] font-semibold text-fg-muted">
              Day {card.subscribedSinceDay}
            </span>
            <span className={`rounded-full border border-border px-2.5 py-1 text-[11px] font-semibold ${card.daysAtRisk > 0 ? 'bg-warning-soft text-warning' : 'bg-positive-soft text-positive'}`}>
        ...
        <div className="mt-auto flex gap-2 border-t border-border pt-4">
          <button
            ...
            className="flex-1 rounded-xl border border-border bg-surface px-3 py-2 text-sm font-semibold text-fg transition-colors hover:bg-surface-strong disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <Link
            ...
            className="flex-1 rounded-xl border border-border bg-positive-soft px-3 py-2 text-center text-sm font-semibold text-positive transition-colors hover:brightness-105"
          ...
          <button
            ...
            className="flex-1 rounded-xl border border-border bg-negative-soft px-3 py-2 text-sm font-semibold text-negative transition-colors hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-50"
