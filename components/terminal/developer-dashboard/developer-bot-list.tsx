...
const statusColors: Record<DeveloperBotSummary['status'], { bg: string; text: string; dot: string }> = {
  ACTIVE: {
    bg: 'bg-positive-soft border-border',
    text: 'text-positive',
    dot: 'bg-positive animate-pulse',
  },
  PAUSED: {
    bg: 'bg-warning-soft border-border',
    text: 'text-warning',
    dot: 'bg-warning',
  },
  DOWN: {
    bg: 'bg-negative-soft border-border',
    text: 'text-negative',
...
export function DeveloperBotList({ bots, activeBotId }: DeveloperBotListProps) {
  ...
  return (
    <article className="glass-strong rounded-2xl p-5 shadow-[var(--shadow-soft)] border border-border relative overflow-hidden group">
      <div className="flex items-center justify-between border-b border-border pb-4 mb-4">
        ...
        <Link
          ...
          className="inline-flex items-center justify-center rounded-lg border border-border bg-surface p-1.5 text-muted transition-colors cursor-pointer hover:bg-surface-strong hover:text-fg"
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full rounded-xl border border-border bg-surface py-2 pl-9 pr-4 text-xs text-fg placeholder:text-muted outline-none transition-all focus:border-[var(--primary-soft)] focus:bg-surface-strong focus:shadow-[0_0_12px_var(--primary-soft)]"
        />
        {searchQuery && (
          <button
      ...
      {/* Bot List Container */}
      <div className="space-y-2.5 max-h-[580px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-[var(--border-base)] scrollbar-track-transparent">
        {filteredBots.length === 0 ? (
          <div className="py-8 text-center rounded-xl border border-dashed border-border bg-surface">
              ...
              >
                <div
                  className={`group/item cursor-pointer rounded-xl border p-3.5 transition-all duration-300 ${
                    isActive
                      ? 'border-border bg-positive-soft'
                      : 'border-border bg-surface hover:bg-surface-strong hover:border-border'
                  }`}
                >
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-border pt-2.5 text-[10px]">
